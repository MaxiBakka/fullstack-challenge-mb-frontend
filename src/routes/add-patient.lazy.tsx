...
import { getLocalStorageOrDefault } from "../utils/data-parser";
...
function FormCreatePatient() {
  ...
  const onSubmit = handleSubmit(async (formData) => {
    const { countryCode, ...restForm } = formData;
    const patientBody = {
      ...restForm,
      phoneNumber: `${countryCode}${restForm.phoneNumber}`,
    };
    ...
    if (status === HTTP_CODES_ENUM.CREATED) {
      const patients = getLocalStorageOrDefault("patients", []);
      localStorage.setItem("patients", JSON.stringify([...patients, data]));
      navigate({ to: "/" });
    }
  });
